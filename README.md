...
Approach
-----------
The program reads a json input file, assigns packages to the closest agent, simulates the deliveries, and writes the report.json file.
...
6. An agent only moves to a warehouse if they have to.

7. In case of a tie during the agent assignment process, we choose the agent with the lower index.
...
10. The best_agent is the one with the minimal efficiency. Similar to the agent selection in case of a tie, the best_agent is selected by choosing the one with the lowest index in case of a tie.
...
14. Only the standard Python libraries were used.
